window.addEventListener(
    ...
    function () {
        ...
        for (let i = 0; i < elems.length; ++i) {
            ...
            const valueData = elem.getElementsByClassName(
                'question-category__value'
            );
            ...
            if (values.length != 2) {
                console.log(
                    "Error! Value does not contain 2 integers separated by a '/'"
                );
            }
            const percentage =
                (parseInt(values[0]) / parseInt(values[1])) * 100;
            const fillerData = elem.getElementsByClassName(
